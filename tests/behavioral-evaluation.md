...
|---|---|---|
| Broad but concise | “Use negation and a list to broadly explore ways a neighborhood tool library could work without paid staff. Keep it easy to scan.” | Scope spans different mechanisms, not renamed variants; every declared candidate appears once with a transformation and implication; shared caveats are not repeated per item. |
| Explicit breadth under a length limit | “Use a chart to cross central/host/cart custody with booked/walk-up/locker/doorstep/weekly-event access. Show all 15 pairs in under 500 words.” | All 15 pairs remain identifiable; short entries preserve meaningful differences; no sampling or silent dropped pairs. |
| Tiny creative request | “Give exactly three strange library premises, no analysis.” | Exactly three distinct premises; no scope report, method lecture, or added assessment that violates the explicit request. |
| Deep versus broad | “Use substitution to deeply investigate one tool-library design's staffing dependency; don't give me a broad list.” | Develops causal dependencies, counterexamples, and revisions within one design rather than increasing candidate count to simulate depth. |
